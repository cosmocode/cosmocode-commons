...
/**
 * Implementation of {@link Rule#and(Rule)}.
 *
 * @since 1.9
 * @author Willi Schoenborn
 * @param <T> generic parameter type
 */
final class AndRule<T> extends AbstractRule<T> {
    ...
    public boolean equals(Object that) {
        if (this == that) {
            return true;
        } else if (that instanceof AndRule<?>) {
            final AndRule<?> other = AndRule.class.cast(that);
