...
/**
 * Implementation of {@link Rule#or(Rule)}.
 *
 * @since 1.9
 * @author Willi Schoenborn
 * @param <T> generic parameter type
 */
final class OrRule<T> extends AbstractRule<T> {

    private final Rule<? super T> left;
    private final Rule<? super T> right;
    
    public OrRule(Rule<? super T> left, Rule<? super T> right) {
    ...
    public boolean equals(Object that) {
        if (this == that) {
            return true;
        } else if (that instanceof OrRule<?>) {
            final OrRule<?> other = OrRule.class.cast(that);
